...
public class AddressBook {
    ...
    private static final String MESSAGE_SORTED = "The Address Book has been sorted.";
    ...
    private static final String COMMAND_ADD_PARAMETERS = "NAME "
                                                      ...
                                                      + PERSON_DATA_PREFIX_DOB + "DATE_OF_BIRTH (YYYY-MM-DD)";
    private static final String COMMAND_ADD_EXAMPLE = COMMAND_ADD_WORD + " John Doe p/98765432 e/johnd@gmail.com d/1994-05-28";
    ...
    private static final String COMMAND_SORT_DESC = "Alphabetically sorts the address book by name";
     ...
     */
    private enum PersonProperty {NAME, EMAIL, PHONE, DOB};
    ...
    /**
     * Indexing for internal checking of Person DoB input.
     */
    private static final int DOB_DAY_INDEX = 2;
    private static final int DOB_MONTH_INDEX = 1;
    private static final int DOB_YEAR_INDEX = 0;
    ...
    */
    ...
    private static String extractPhoneFromPersonString(String encoded) {
        ...
        if (indexOfPhonePrefix > Math.max(indexOfEmailPrefix, indexOfDobPrefix)) {
            return removePrefixSign(encoded.substring(indexOfPhonePrefix, encoded.length()).trim(),
                    PERSON_DATA_PREFIX_PHONE);
        // phone is start arg, target is from own prefix to next prefix
        } else if (indexOfPhonePrefix < Math.min(indexOfEmailPrefix, indexOfDobPrefix)) {
            return removePrefixSign(
                    encoded.substring(indexOfPhonePrefix, Math.min(indexOfEmailPrefix, indexOfDobPrefix)).trim(),
                    PERSON_DATA_PREFIX_PHONE);
        // phone is middle arg, target is from own prefix to next prefix
        } else {
            return removePrefixSign(
                    encoded.substring(indexOfPhonePrefix, Math.max(indexOfEmailPrefix, indexOfDobPrefix)).trim(),
                    PERSON_DATA_PREFIX_PHONE);
        }
    }
    ...
    private static String extractEmailFromPersonString(String encoded) {
        ...
        if (indexOfEmailPrefix > Math.max(indexOfPhonePrefix, indexOfDobPrefix)) {
            return removePrefixSign(encoded.substring(indexOfEmailPrefix, encoded.length()).trim(),
                    PERSON_DATA_PREFIX_EMAIL);
        // email is start arg, target is from own prefix to next prefix
        } else if (indexOfEmailPrefix < Math.min(indexOfPhonePrefix, indexOfDobPrefix)) {
            return removePrefixSign(
                    encoded.substring(indexOfEmailPrefix, Math.min(indexOfPhonePrefix, indexOfDobPrefix)).trim(),
                    PERSON_DATA_PREFIX_EMAIL);
        // email is middle arg, target is from own prefix to next prefix
        } else {
            return removePrefixSign(
                    encoded.substring(indexOfEmailPrefix, Math.max(indexOfPhonePrefix, indexOfDobPrefix)).trim(),
                    PERSON_DATA_PREFIX_EMAIL);
        }
    }
    ...
    private static String extractDobFromPersonString(String encoded) {
        ...
        // edob is last arg, target is from prefix to end of string
        if (indexOfDobPrefix > Math.max(indexOfPhonePrefix, indexOfEmailPrefix)) {
            return removePrefixSign(encoded.substring(indexOfDobPrefix, encoded.length()).trim(),
                    PERSON_DATA_PREFIX_DOB);
            // dob is start arg, target is from own prefix to next prefix
        } else if (indexOfDobPrefix < Math.min(indexOfPhonePrefix, indexOfEmailPrefix)) {
            return removePrefixSign(
                    encoded.substring(indexOfDobPrefix, Math.min(indexOfPhonePrefix, indexOfEmailPrefix)).trim(),
                    PERSON_DATA_PREFIX_DOB);
            // dob is middle arg, target is from own prefix to next prefix
        } else {
            return removePrefixSign(
                    encoded.substring(indexOfDobPrefix, Math.max(indexOfPhonePrefix, indexOfEmailPrefix)).trim(),
                    PERSON_DATA_PREFIX_DOB);
        }
    }
     ...
     *
     * @param person HashMap<Enums, String> representing the person (used in internal data)
     */
    ...
    /**
     * Returns true if the given string as a valid birth date
     *
    ...
    private static boolean isPersonDobValid(String dob) {
        ...
        if (dateProperties.length == 3) {
            ...
            int day = Integer.parseInt(dateProperties[DOB_DAY_INDEX]);
